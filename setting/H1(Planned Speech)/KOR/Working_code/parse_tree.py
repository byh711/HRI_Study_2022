...
def get_utterance_from_abstract(abstract):
    ...
    if cf.priority_utterance_score[abstract] <= cf.CHATTINESS:
        ...
        # 발화 내용이 담긴 리스트를 만들어준다.
        utt_list = ast.literal_eval(cf.RESPONSE_UTTS[abstract])  # 리스트 형으로 저장해준다.

        return random.choice(utt_list)    # 여러 발화문장 중 아무거나 하나 뽑아준다.
...
def parse_day_subtree(current_state, initial_state):
    ...
    if current_state['Phase'] == 'day':
        ...
        # text 파일에 저장되어 있는 발화 중 하나를 선택한다.
    ...
    if current_state['Player_Xloc'] != '999':
        ...
        if cf.status['Phase'][-1] == 'day' or cf.status['Phase'][-1] == 'dusk':
            ...
            if len(cf.status['Player_Xloc']) >= 40:
                if cf.status['Player_Xloc'][-1] == cf.status['Player_Xloc'][-40]:
...
def parse_decision_tree(current_state, initial_state):
    '''
    parse(분석하다) the decision tree to find the right action for the current state
    synthesize(통합하다) the utterance and play it back
    '''
    ...
    # 스레드 실행
    # tts.synthesize_utt(utterance)를 멀티스레드로 실행
    # daemone: 백그라운드에서 실행되는 스레드. 프로그램이 종료될때 함께 종료
